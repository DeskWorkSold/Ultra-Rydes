...
export default function PassengerDetailsEdit({navigation, route}) {
  ...
  const [mobileNumber, setMobileNumber] = useState(
    passengerData.mobileNumber ? passengerData.mobileNumber : null,
  );
  ...
  const askScreenHandler = async () => {
    if (
      ...
      Alert.alert('Confirmation', 'Are you sure you want to make changes', [
        {
          text: 'Cancel',
          onPress: () => {
            return false;
  ...
  const updateData = async () => {
    ...
    try {
      ...
      firestore()
        ...
        .doc(CurrentUser.uid)
        .update({
          profilePicture: IsProfilePicChanged
            ? profilePicture
