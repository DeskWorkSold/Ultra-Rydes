...
export default function DriverVehicleEdit({navigation, route}) {
  ...
  useEffect(() => {
    async function fetchUrl() {
      ...
      if (vehicleDetails.vehiclePicFront.length) {
        const vehicleFrontPicUrl = await storage()
          .ref(vehicleDetails.vehiclePicFront)
          .getDownloadURL();
        setVehiclePicFront(vehicleFrontPicUrl);
      }

      if (vehicleDetails.vehiclePicBack.length) {
        const vehicleBackPicUrl = await storage()
          .ref(vehicleDetails.vehiclePicBack)
          .getDownloadURL();
        setVehiclePicBack(vehicleBackPicUrl);
      }

      if (vehicleDetails.vehicleNumPlatePic.length) {
        const vehicleNumPlatePicUrl = await storage()
          .ref(vehicleDetails.vehicleNumPlatePic)
          .getDownloadURL();
  ...
  const [isVehNumPlatePicUpdated, setIsVehicleNumPlatePicUpdated] = useState(
    false,
  );
  //check status end
      ...
      firestore()
        ...
        .update({
          vehicleDetails: {
            ...
            vehiclePicFront: isVehFrontPicUpdated
              ? vehiclePicFront
              : vehicleData.vehiclePicFront,
            vehiclePicBack: isVehBackPicUpdated
              ? vehiclePicBack
              : vehicleData.vehiclePicBack,
            vehicleNumPlatePic: isVehNumPlatePicUpdated
              ? vehicleNumPlatePic
              : vehicleData.vehicleNumPlatePic,
          },
        })
        .then(() => {
          setLoading(false);
